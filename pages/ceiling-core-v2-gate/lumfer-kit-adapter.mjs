import {packPurchasePlan} from './procurement-v2.mjs';
...
function normalizedLine(db, productId, requiredQty, usdBynRate, options = {}) {
  const product = byId(db, productId);
  const rawUnit = priceByn(product, usdBynRate);
  const amount = positive(requiredQty);
  const packSize = Math.max(1, positive(options.packSize) || 1);
  const packed = packSize > 1;
  const purchase = packed
    ? packPurchasePlan(amount, packSize, rawUnit)
    : {
        requiredQty:amount,
        packSize:1,
        packCount:amount,
        purchaseQty:amount,
        remainderQty:0,
        purchaseUnitCost:rawUnit,
        unitCost:rawUnit,
        total:amount * rawUnit,
      };
  return {
    source: 'lumfer-auto',
...
function mergeSameProduct(lines) {
  ...
  for (const line of lines) {
    ...
    if (!prev) map.set(key, {...line});
      ...
      prev.requiredQty += line.requiredQty;
      prev.qty += line.qty;
      prev.purchaseQty += line.purchaseQty;
      prev.remainderQty += line.remainderQty;
      prev.packCount += line.packCount;
...
export function buildLumferAutoKit({room, electrical, fasteners, db, usdBynRate = 0} = {}) {
  ...
  if (fastenerQty) {
    lines.push(normalizedLine(db, LUMFER_ITEM_IDS.wallDowelChapay6x35Pack1000, fastenerQty, usdBynRate, {
      role:'wall-dowel', unit:'шт.', packSize:1000, fallbackName:'Дюбель «Чапай» 6×35',
    }));
    lines.push(normalizedLine(db, LUMFER_ITEM_IDS.wallScrew35x35Pack1000, fastenerQty, usdBynRate, {
      role:'wall-screw', unit:'шт.', packSize:1000, fallbackName:'Саморез 3,5×35',
    }));
  }
