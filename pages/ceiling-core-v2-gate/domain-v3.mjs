...
import {reinforcedFastenerPlan, validateRoomQuote} from './procurement-v2.mjs';
...
export function createRoom(name='Помещение 1') {
  return {
    ...
    pricing: {
      ...
      laborCostPerM2: 12,
      wastePct: 8,
      profileProductId: null,
      fastenerExtraPerCorner: null,
      fastenerExtraPerJoint: null,
    },
  };
}
...
export function geometrySummary(room) {
  ...
  const angles = points.length >= 3 ? points.map(() => 90) : [];
  return {grossArea,nicheArea,area,perimeter:perimeter+nichePerimeter,basePerimeter:perimeter,nichePerimeter,points,angles};
}
...
export function profilePurchasePlan(product, requiredM, usdBynRate=0) {
  ...
  return {...stock,pricingBasis,chargeQty,dealerUnit:dealer?.unit||product?.default_unit||'',dealerUnitPriceByn,cost};
}

export function featureCostBreakdown(f={}) {
  const qty=Math.max(1,+f.qty||1);
  const length=Math.max(0,+f.length||0);
  const laborUnit=Math.max(0,+f.unitCost||0);
  const materialUnit=Math.max(0,+f.materialUnitCost||0);
  const cornerCount=Math.max(0,+f.cornerCount||0);
  const cornerUnit=Math.max(0,+f.cornerUnitCost||0);
  let labor=0, material=0, corners=0;
  if (f.type==='track' || f.type==='cornice') {
    labor=length*laborUnit;
    material=length*materialUnit;
    corners=cornerCount*cornerUnit;
  } else {
    labor=qty*laborUnit;
    material=qty*materialUnit;
  }
  return {qty,length,labor,material,corners,total:labor+material+corners};
}

export function featureCost(f) {
  return featureCostBreakdown(f).total;
}
...
export function calculateRoom(room, db={products:[]}, settings={usdBynRate:0}) {
  ...
  const baseFasteners=profile ? wallFastenerPlan(geom.perimeter) : wallFastenerPlan(0);
  const fasteners=reinforcedFastenerPlan(baseFasteners,{
    cornerCount:profile ? geom.angles.length : 0,
    jointCount:profile ? Math.max(0,profileStock.pieces-1) : 0,
    extraPerCorner:room.pricing?.fastenerExtraPerCorner,
    extraPerJoint:room.pricing?.fastenerExtraPerJoint,
  });
  ...
  const features=(room.features||[]).map(x=>({...x,costBreakdown:featureCostBreakdown(x),total:featureCost(x)}));
  ...
  const validation=validateRoomQuote({room,fasteners,electrical,autoKit});
  return {room,geom,factor,membraneQty,membraneCost,laborCost,profile,profileQty,profileStock,profileCost,fasteners,electrical,autoKit,autoMaterialsCost,materials,materialsCost,features,featuresCost,cost,validation,isComplete:validation.isComplete};
}
...
export function calculateProject(project, db={products:[]}) {
  ...
  const issues=rooms.flatMap(r=>(r.validation?.issues||[]).map(x=>({...x,roomId:r.room.id,roomName:r.room.name})));
  const isComplete=rooms.every(r=>r.isComplete);
  return {rooms,cost,clientTotal,profit,margin,isComplete,issues,totalArea:rooms.reduce((s,r)=>s+r.geom.area,0),totalPerimeter:rooms.reduce((s,r)=>s+r.geom.perimeter,0)};
}
