...
export function buildElectricalPlan(features = [], options = {}) {
  ...
  const lineIds = new Set(electrical.map(f => f.electricalLineId).filter(Boolean));
  const lineCount = pointCount ? Math.max(1, lineIds.size || positive(options.lineCount) || 1) : 0;
